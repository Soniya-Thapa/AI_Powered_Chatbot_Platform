...
import { NoChatsEmpty } from '@/components/empty.state';
...
export default function Sidebar({ onChatSelect }: SidebarProps) {
  ...
  const { chats, currentChat, loading, filteredChats, searchQuery } = useAppSelector((state) => state.chat);
  const { user } = useAppSelector((state) => state.auth);

  // Use filteredChats when searching, otherwise use all chats
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading && chats.length === 0 ? (
            ...
            <NoChatsEmpty onCreateChat={handleNewChat} />
          ) : (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-1 hover:bg-red-100 dark:hover:bg-red-900/20 rounded transition-opacity disabled:opacity-50"
                  >
